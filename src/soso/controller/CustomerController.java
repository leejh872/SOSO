...
public class CustomerController {
	...
	@RequestMapping(value = "reg", method = RequestMethod.POST)
	   public String reg(@RequestParam(value = "file") MultipartFile[] files, HttpServletRequest request, HttpSession session) throws IOException {
...
*/
	       ...
	       String path = ctx
	       .getRealPath("/SOSO/WebContent/resource/customer/upload"); /* 물리경로 내가 업로드할 파일 경로 */
	       ...
	       for(int i=0; i<files.length; i++)
	         {
	            ...
	            String fpath = path + File.separatorChar + fname;
	...
	public String detailCmtDel(String ccode) {
	  ...
	  @RequestMapping("detail-like") public String detailLike(String post_code, String email) {
	  ...
	  // 로그인이 되어 있다면 
	  } else { // LIKE를 누른적이 있는지 확인하고 추가하거나 감소 시킨다. 
*/	  //post_code, email로 User의 like 값을 가져온다. 
	  ...
	  // like값을 찾았다면 // is_like(개인 like 0~1), post_code, user_email 
	  if (like != null) { 
		  if (like.getIs_like() == 0) likeDao.updateLike(1, post_code, email); 
		  else likeDao.updateLike(0, post_code, email); 
	  } 
	  return "customer.detail";
	 }
	
	@RequestMapping("likespage")
	public String LikesPage(Model model, HttpServletRequest request, HttpSession session) {
		// 로그인 안됨
		/*session = request.getSession();
		String user_email = (String) session.getAttribute("email");*/
		String user_email = "test@naver.com";
		System.out.println("myemail: " + user_email);
		List<PhotoFileLikeModel> likephotoList = likeDao.getLikePhoto(user_email);
	...
	public String myPage(Model model, HttpServletRequest request, HttpSession session) {
		...
		List<String> myphotoList = postDao.getMyPhoto(email);
